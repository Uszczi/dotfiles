...
function fish_command_timer_postexec -e fish_postexec
    ...
    if set -q fish_command_timer_min_cmd_duration; and [ "$fish_command_timer_min_cmd_duration" -gt "$CMD_DURATION" ]; and begin
            [ "$last_status" -eq 0 ]; or not set -q fish_command_timer_status_enabled; or [ "$fish_command_timer_status_enabled" -eq 0 ]
        end
        return
    end
    ...
    set -l status_str ""
    if begin
            set -q fish_command_timer_status_enabled; and [ "$fish_command_timer_status_enabled" -ne 0 ]
        end
    ...
    if begin
            [ $last_status -eq 0 ]; and set -q fish_command_timer_success_color; and [ -n "$fish_command_timer_success_color" ]
        end
        set status_str_colored (set_color $fish_command_timer_success_color)"$status_str"(set_color normal)
    else if begin
            [ $last_status -ne 0 ]; and set -q fish_command_timer_fail_color; and [ -n "$fish_command_timer_fail_color" ]
        end
